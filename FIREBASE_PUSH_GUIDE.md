...
이 문서는 캠프 PWA에 **관리자 공지 입력 → 전체 사용자 푸시 알림** 기능을 추가하는 전체 과정을 다룬다. Claude Code에서 이 레포를 열고 "FIREBASE_PUSH_GUIDE.md대로 구현해줘"라고 요청하면 동일하게 구현된다. 직접 따라 해도 무방하다. 단, **Firebase 콘솔 설정인 1단계는 반드시 직접 수행해야 한다.**
