...
- **푸시 흐름**: Admin이 `announcements` 문서 생성 → Functions `onDocumentCreated`(`sendAnnouncement`) 트리거 → 전체 토큰에 **data-only** 페이로드 발송 → `src/sw.js`가 알림 표시 + 배지 증가. 수정 재발송은 `onDocumentUpdated`(`resendAnnouncement`)가 `resendAt` 변경 시에만 동일 발송 경로(`sendToAllTokens`)를 탄다.
- **인증**: 익명 로그인(참가자, 프로필용) + 이메일 로그인(관리자). `isAdmin = !!user.email`.
- **공지 데이터 모델**: `{ title, body, blocks[], pinned, createdAt }`(+ 수정 시 `updatedAt`, 재발송 체크 시 `resendAt`). `blocks`는 `text`/`image`/`file`/`link` 타입의 순서 배열. `body`는 첫 텍스트 블록(미리보기·푸시 본문 하위호환). 첨부는 Storage 업로드, 이미지는 클라이언트 리사이즈.
...
- 공지 **수정**은 기본적으로 푸시를 보내지 않는다(createdAt 유지, 목록 순서 보존). 다만 관리자가 편집 화면에서 "다시 보내기"를 체크하면 update에 `resendAt`(serverTimestamp)를 심고, Functions `onDocumentUpdated`(`resendAnnouncement`)가 `resendAt` 변경을 감지해 1회만 재발송한다. `resendAt`를 건드리지 않는 수정(고정 토글 등)은 푸시가 나가지 않는다. 이 동작을 깨지 않는다.
