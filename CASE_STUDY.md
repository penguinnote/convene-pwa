...
**결과는 항목별로 달랐다.** 공지 열람 94%(75명·941회), 푸시 권한 허용률 98%, 3일 이상 방문 81%로 세 지표는 목표를 달성했고, 설치율은 88%로 목표 90%에 근접 미달했다.

다만 이 수치는 **최초 집계를 한 차례 폐기한 뒤 얻은 것**이다. 익명 로그인 구조에서 재설치·기기 추가마다 새 uid가 생성되어, 최초 집계는 uid를 사람으로 간주하고 있었다. 설치율은 61%, 1일차 이후 이탈은 55%로 나왔으나 둘 다 실제 현상이 아니라 집계 방식이 만든 수치였다. 닉네임 기준으로 uid를 병합해 재산출한 결과 설치율 88%, DAU 감소폭 14%로 바뀌었다.
...
**사후 검증 — 이 결정에는 비용이 따랐다.** 재설치·기기 추가마다 새 uid가 생성되어, 등록 명단 96명에 대해 프로필 등록 123건, 방문 uid 172개가 집계됐다. 캠프 종료 후 닉네임·목장 기준으로 uid를 병합해 사람 단위 지표를 복원했으나(123건 → 80명), 닉네임을 등록하지 않은 uid 53개는 끝내 귀속시키지 못했다. 상세 내용은 8장에 기술한다.
...
캠프 종료 후 닉네임과 목장을 정규화해 uid를 사람 단위로 병합하는 로직을 대시보드에 추가하고 재산출했다. 프로필 123건이 **사람 80명**으로 수렴했다(중복 43건, 다중 uid 보유 38명).
...
원본 데이터는 동일하며 집계 방식만 변경했다. 대시보드에는 두 기준을 전환하는 토글과 병합 내역 확인 기능을 함께 넣어, 동명이인이 잘못 묶였는지 검토할 수 있도록 했다. 아래 성적표는 재산출 기준이다.

### 성적표

분모를 두 가지로 병기했다. **앱 참여자 80명**은 프로필을 등록한 고유 인원이고, **등록 명단 96명**은 캠프 참가자 전체다.

| 지표 | 목표 | 앱 참여자 80명 기준 | 등록 명단 96명 기준 | 판정 |
| --- | --- | --- | --- | --- |
| 공지 열람 | 정성 목표 "미확인자 없음" | **94%** (75/80) · 941회 | 78% (75/96) | ✅ 달성 |
| 푸시 권한 허용률 | ≥ 70% | **98%** (50/51) | 동일 | ✅ 달성 |
| 4일 리텐션 (3일 이상) | ≥ 60% | **81%** (65/80) | 68% (65/96) | ✅ 달성 |
| 설치율 | ≥ 90% | **88%** (70/80) | 73% (70/96) | ⚠️ 근접 미달 |
| 조 편성 완료율 | ≥ 80% | 70% (56/80) | 58% (56/96) | ❌ 미달 |
| 온보딩 완료율 | ≥ 90% | — | **83%** (80/96) | ⚠️ 근접 미달 |

**보조 지표** 세션 1,616회 · 화면 조회 8,221회 · 공지 발송 31건 · 등록 토큰 60개 · 관리자 5명 · iOS 41 / Android 37 / 기타 2
...
**"QR 설치형 PWA로 100명 규모의 설치 마찰을 감당할 수 있다"** — 대체로 검증됐다. 앱을 사용한 80명 중 70명(88%)이 홈 화면 설치까지 진행해 목표 90%에 근접했다. 네이티브 앱의 심사·용량·업데이트 마찰을 피하면서 이 수준을 확보한 것은 선택의 타당성을 뒷받침한다.

**"공지 전달이 핵심 가치다"** — 검증됐다. 앱 참여자의 94%인 75명이 941회 열람했으며 1인당 평균 약 12회다. 4일간 31건의 공지가 발송됐고, 앱 재방문을 유발한 주요 요인은 공지였다.

**"익명 로그인으로 진입 마찰을 제거하면 활성화가 상승한다"** — 방향은 검증됐다. 로그인 절차가 없어 앱에 접근한 인원 대부분이 프로필 등록까지 도달했다. 다만 대조군이 없어 인과관계로 단정할 수 없으며, 아래에 기술한 측정 비용이 발생했다.

### 반증된 가설 ① — "권한 허용률이 높으면 푸시가 도달한다"

허용률은 98%였으나 실제 토큰 보유는 60개에 그쳤다. 퍼널로 분해하면 원인이 확인된다.
...
사전 가정 중 실제와의 차이가 가장 큰 항목이다. 등록 명단 96명에 대해 프로필 등록 123건, 방문 uid 172개가 집계됐고, uid 수가 실인원의 1.3~1.8배 수준이었다.
...
사후 병합으로 상당 부분을 복원했으나 완전하지는 않다. 닉네임을 등록하지 않은 uid 53개는 귀속시킬 근거가 없어 남았고, 이 때문에 "앱을 연 사람이 몇 명인지"는 80~133명 범위로만 좁혀진다. 근본적인 해결책은 사후 병합이 아니라 uid와 개인을 처음부터 연결하는 장치다.
...
| 날짜 | DAU (uid 기준) | DAU (사람 기준) | 세션 | 화면 조회 | 세션당 화면 |
| --- | --- | --- | --- | --- | --- |
| 7/29 (1일차) | 146 | **70** | 642 | 3,935 | 6.1 |
| 7/30 | 80 | **68** | 396 | 1,971 | 5.0 |
| 7/31 | 75 | **66** | 331 | 1,464 | 4.4 |
| 8/01 (4일차) | 66 | **60** | 247 | 851 | 3.4 |

사람 기준 DAU는 70에서 60으로 **14% 감소**에 그쳤다. 1일차의 146은 설치가 집중되며 새 uid가 대량 생성된 결과였다. 3일 이상 방문자가 65명(81%)이라는 리텐션 수치와도 일치한다.
...
- **성능 제약이 기술이 아닌 온보딩에 있을 수 있다.** 푸시 도달의 한계는 발송 로직이 아니라 설치·프롬프트 단계에서 발생했다. 지표를 분리하지 않았다면 원인을 잘못 진단했을 가능성이 있다.
...
3. **푸시 프롬프트 도달률 개선** — 설치한 70명 중 51명만 권한 프롬프트에 도달했다. 설치 직후 권한 요청을 명시적 단계로 배치한다.
4. **미사용 링크 정리** — 라이브 카드의 `schedule`·`versesTab` 링크를 제거한다. 기능 추가와 마찬가지로 제거 역시 결정 사항이다.
5. **조 편성 분모 기록** — 레크레이션 실참여 인원을 이벤트로 남겨 완료율을 산출 가능하게 한다.
6. **측정 분모 선확정** — 지표 정의서에 분자·분모·이벤트명을 함께 기재한 후 이벤트를 구현한다.
