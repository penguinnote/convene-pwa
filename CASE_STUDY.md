...
5. **조 편성 완료율의 분모는 전체 인원**이다. 사전 분모였던 레크레이션 참여자를 기록하지 않았으므로 실제 완료율은 표의 값보다 높다.
...
1. **설치 안내 온보딩 팝업** — 첫 접속 시 홈 화면 추가 방법을 안내하는 팝업을 띄운다. iOS·Android 각각의 절차를 보여주고 "24시간 동안 보지 않기"와 "다시 보지 않기"를 제공해, 이미 설치한 사용자에게 반복 노출되지 않게 한다. 설문에서 "안내는 받았지만 설치 방법을 몰랐다" 3건과 "QR로 접속했으나 웹으로 열렸다"는 응답이 나왔고, 알림 미수신자 4명이 전원 미설치자였다. 운영진 논의를 거쳐 최우선 과제로 정했다.
2. **공지 제목의 정보 완결성** — 응답자의 41%가 "제목만 보고 넘겼다"고 답했다. 본문을 열게 만들기보다 제목만으로 필요한 정보가 전달되도록 작성 원칙을 세우고, 하루 7.8건이던 발송량을 절반 이하로 줄인다.
...
7. **말씀 한·영 전환 노출 개선** — 설문에서 39%가 이 기능의 존재를 몰랐다. 발견성 개선이 필요한 유일한 기능이다.
