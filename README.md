...
- **데스크톱 레이아웃** — 넓은 화면(태블릿·노트북, ≥768px)에선 전체 폭 수채화 히어로 + 가로 탭 내비의 데스크톱 셸과 홈 대시보드(라이브 와이드 카드, 공지·오늘 일정 2열, 프로필·INFO 정보 레일)를 제공한다. 일정(넓은 타임라인)·말씀(2단 마스터-디테일)·방배정(섹션 카드 그리드)·공지·식단·플레이리스트·관리자·온보딩도 각각 넓은 화면용으로 배치하고, "정보" 탭 콘텐츠는 홈 레일로 옮겼다. 폰(<768px)은 기존 모바일 UI(하단 5개 탭, 중앙 열)를 그대로 유지한다.
