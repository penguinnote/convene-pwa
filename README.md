...
- **데스크톱 레이아웃** — 넓은 화면(태블릿·노트북, ≥768px)에선 전체 폭 수채화 히어로 + 가로 탭 내비의 데스크톱 셸과 홈 대시보드(라이브 와이드 카드, 공지·오늘 일정 2열, 프로필·INFO 정보 레일)를 제공한다. 폰(<768px)은 기존 모바일 UI(하단 탭, 중앙 열)를 그대로 유지한다.
