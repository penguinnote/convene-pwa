...
이 문서는 Convene에 **관리자 공지 입력 → 전체 사용자 푸시 알림** 기능을 추가하는 전체 과정을 다룬다.
...
- **알림이 2번 도착한다**: 메시지를 `data` 페이로드로 보내 서비스워커가 한 번만 표시하게 한다. `notification` 페이로드를 함께 보내면 FCM이 자동으로 표시하고 서비스워커가 또 표시해 중복된다.
- **아이폰에서 푸시가 오지 않는다**: 아이폰은 홈 화면에 설치한 뒤 그 앱으로 실행한 상태에서만 푸시를 수신한다. Safari 탭으로 연 상태에서는 권한 요청과 수신이 동작하지 않는다.
- **재설치 후 알림이 오지 않는다**: 앱을 재설치하면 알림 권한과 토큰이 초기화된다. 앱에서 "알림 받기"로 다시 등록해야 한다.
- **앱을 보는 중에 팝업이 뜨지 않는다**: 푸시 팝업은 앱이 백그라운드일 때만 표시되며, 앱을 보고 있는 동안에는 표시되지 않는다. 포그라운드에서는 시스템 푸시 대신 Firestore 구독 기반 인앱 토스트가 화면 상단에 표시된다.
- **안드로이드에서 알림을 탭해도 앱이 열리지 않는다**: 서비스워커에 `notificationclick` 핸들러를 두어 알림 탭 시 열린 창을 포커스하거나 새로 연다. iOS는 별도 핸들러 없이 탭하면 앱이 열린다.
- **절전 상태에서 알림이 늦게 온다**: webpush 헤더에 `Urgency: high`를 지정해 삼성과 안드로이드 절전 상태에서도 즉시 전달되게 한다.
- **여러 공지가 하나로 합쳐진다**: 알림마다 공지 문서 id를 고유 `tag`로 부여해 여러 공지가 서로 덮어쓰지 않고 쌓이도록 한다. 고정 `tag`를 쓰면 마지막 알림이 이전 알림을 덮는다.
- **삼성 기기에서 배너 팝업이나 화면 깨우기가 동작하지 않는다**: 배너 표시와 화면 깨우기는 기기 알림 설정의 영향을 받아 코드로 완전히 제어할 수 없다. 온보딩에서 알림 설정 안내로 대응한다.
